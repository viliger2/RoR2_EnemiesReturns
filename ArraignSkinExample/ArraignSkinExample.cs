...
namespace ArraignSkinExample
{
    ...
    public class ArraignSkinExample : BaseUnityPlugin
    {
        ...
        public void Awake()
        {
            ...
            {
                EnemiesReturnsModCompat.RegisterModdedSkin(AnointedSkins_onGatherModdedAnointedSkins);
            }
        }

        private void AnointedSkins_onGatherModdedAnointedSkins()
        {
            AddAnointedSkinFromContentPack();
        }
        ...
        public static void Init()
        {
            if (!EnemiesReturnsModCompat.enabled)
            {
                AddJustSkinFromCatalog();
            }
        }

        // this entire thing is pretty much copy pasted from SkinBuilder
        private static void AddJustSkinFromCatalog()
        {
            string skinDefName = "NormalSkinExampleBandit2SkinDef";
            try
            {
                ...
                var skinController = AddSkin(gameObject, skinDefName);
                if (skinController)
                {
            ...
            {
                Log.Warning((object)("Failed to add normal \"" + skinDefName + "\" skin to \"" + BodyName + "\""));
                Log.Error((object)ex2);
            }
        }

        private static void AddAnointedSkinFromContentPack()
        {
            string skinDefName = "ArraignSkinExampleBandit2SkinDef";
            try
            {
                var collection = RoR2.ContentManagement.ContentManager._bodyPrefabs.Where(gameObject2 => gameObject2.name == BodyName);
                ...
                AddSkin(collection.First(), skinDefName);
            }
            catch (Exception ex2)
            {
                Log.Warning((object)("Failed to add Anointed \"" + skinDefName + "\" skin to \"" + BodyName + "\""));
                Log.Error((object)ex2);
            }
        }

        private static ModelSkinController AddSkin(GameObject gameObject, string skinDefName)
        {
            ...
            if (EnemiesReturnsModCompat.enabled)
            {
                skin = EnemiesReturnsModCompat.CreateHiddenSkinDef(BodyName, skin, true);
            }
