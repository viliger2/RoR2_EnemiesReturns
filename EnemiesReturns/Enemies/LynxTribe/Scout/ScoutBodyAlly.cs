...
{
    public class ScoutBodyAlly : ScoutBody
    {
        public new struct SpawnCards
        {
            public static CharacterSpawnCard cscLynxScoutAlly;
        }
        ...
        public BuffDef CreateSpeedBuffDef(Sprite sprite)
        {
            BuffDef buff = ScriptableObject.CreateInstance<BuffDef>();
            (buff as ScriptableObject).name = "bdLynxSpeed";
            if (sprite)
            {
        ...
        protected override SkinDef[] CreateSkinDefs(GameObject modelPrefab)
        {
            ...
            SkinDefs.Ally = Utils.CreateSkinDef("skinLynxScoutAlly", modelPrefab, defaultRender);
